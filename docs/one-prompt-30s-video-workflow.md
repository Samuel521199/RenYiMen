...
- 不再把每个关键帧当成一个镜头的中点。
- 视频不固定拆成 6 段；大模型按剧情节拍、信息密度、场景变化和动作连续性决定 N 段。
- 系统先规划并生成 N+1 个边界关键帧，分别对应整条时间轴上的首尾帧节点。
- 第 1 段视频使用关键帧 1 作为首帧、关键帧 2 作为尾帧。
- 第 2 段视频使用关键帧 2 作为首帧、关键帧 3 作为尾帧。
- 依此类推，第 N 段视频使用关键帧 N 作为首帧、关键帧 N+1 作为尾帧。
...
- 默认输出：30 秒，9:16，N+1 个边界关键帧，N 段视频；每段 3-15 秒，具体数量和时长由大模型按剧情决定
...
flowchart TD
  ...
  B --> C["LLM Planner: 剧本解析 + N+1 个边界关键帧 + N 段首尾帧视频规划"]
  C --> D["剧本与关键帧计划审核"]
  D --> E["图片生成: N+1 个边界关键帧"]
  E --> F["关键帧图片审核"]
  F --> G["首尾帧视频生成: N 段视频"]
  G --> H["片段质检与单段重试"]
  H --> I["视频合成: N 段拼接 + 字幕/BGM/Logo"]
  I --> J["30s 成片预览与导出"]
```
...
- `VideoSegment`：由相邻两个关键帧生成的视频段，共 N 段。
...
| `KEYFRAME_GENERATING` | 正在生成 N+1 张边界关键帧 | 查看进度 |
| `KEYFRAME_REVIEW` | 等待用户审核关键帧 | 重生成、替换、锁定 |
| `SEGMENT_GENERATING` | 正在生成 N 段首尾帧视频 | 查看每段进度 |
...
- 第 1 个关键帧 `timeSeconds = 0`，最后一个关键帧 `timeSeconds = durationSeconds`。
...
图片生成阶段的对象从“镜头中点图”改为“N+1 个边界关键帧图”。
...
- 总时长：MVP 固定 30s。
- 关键帧数量：N+1，由模型输出的 `segments.length` 推导。
- 视频段数量：N，由模型按剧情节拍、场景变化、动作连续性和信息密度决定。
- 视频比例：9:16 / 16:9 / 1:1。
...
| `POST` | `/api/video-projects/[id]/plan` | 生成或重生成剧本、N+1 个边界关键帧、N 个片段规划 |
...
| `POST` | `/api/video-projects/[id]/approve-keyframes` | 确认 N+1 个边界关键帧，进入视频片段生成 |
...
| `POST` | `/api/video-projects/[id]/approve-segments` | 确认 N 个片段，进入合成阶段 |
...
目标：用户一句话生成可编辑的 N+1 个边界关键帧规划和 N 个片段规划。
...
- 用户输入一句话后，得到 N+1 个边界关键帧节点和 N 个片段规划。
- 每个关键帧都有图片 prompt。
