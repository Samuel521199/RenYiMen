...
用户一句话 + 参数 + 参考图
  -> 阶段 1：Planning Architect
       ...
       输出 Anchor Manifest
       输出 Narrative Event Layer
       输出 Anchor State Timeline
       输出 Audio Bible
  ...
  -> 阶段 2A：Storyboard Artist
       输出 Storyboard Brief
       输出 Camera Graph 初稿
       输出 Final Transition Plan 初稿

  -> 阶段 2B：Shot Decomposer
...
结合 ViMax 的优点和我们当前项目的实际链路，后续重构时不要只把“提示词写得更长”，而应把以下八类产物变成结构化输出和后端可校验对象：

1. `narrativeEvents`：独立剧情事件层。先回答故事发生了哪些因果事件，再由事件推导 segment，避免用户一句话直接跳到镜头时间线。
2. `anchorStateTimeline`：动态状态注册表。把“不能变化的身份/外观”和“可以按剧情变化的位置/持有者/状态”分开，解决产品复制、道具凭空出现、姿态无理由变化。
3. `storyboardBrief` + `segmentRenderDescriptions`：阶段 2 内部拆成 Storyboard Artist 和 Shot Decomposer 两个子任务。前者决定每段看什么，后者决定首帧、尾帧、运动和检查点如何实现。
4. `referenceSelectionOutputs`：参考图选择必须有确定性算法、数量上限、评分和淘汰原因，不能只再问一次大模型。
5. `cameraGraph`：用 `CameraRelation` 描述机位关系，废弃 `is_parent_fully_covers_child` 这类不准确的覆盖判断。
6. `transitionReferencePlan` + `finalTransitionPlan`：前者解决新机位关键帧如何保持空间一致，后者解决最终成片段间如何衔接。
7. `artifactMetadata`：每个中间产物、图片、视频、prompt 编译结果都要有 revision、输入哈希、版本、依赖和 dirty 状态，支持真正局部恢复。
8. `generationQualityReports`：生成后必须评估身份、布局、prompt 对齐、连续性、一镜到底和伪影问题，再决定是否进入下一阶段或局部重试。

这八个产物是对当前三阶段架构的增强，不是把三阶段改成八阶段。它们可以先全部落在 `planJson` 里，等 UI 和后端稳定后再拆成独立表。
...
### 5.4 独立剧情事件层：Narrative Events

当前方案不能让模型从用户一句话直接跳到 segment。中间必须先形成“剧情事件层”，也就是一组 Narrative Beat。它不是镜头，也不是最终分镜，而是回答：这条视频在因果上发生了哪些事件、每个事件的目标是什么、哪些状态被改变。
...
它的作用是让 segment 拆分有依据，而不是只靠模型“感觉”。后续拆分 segment 时必须参考：
...
对于 30 秒广告，事件层通常只有 3 到 5 个 beat，例如“品牌/场景建立 -> 主角接触产品 -> 产品使用/玩法展示 -> 结果强化 -> 结尾 CTA”。即使很短，也应保留这一层，因为它能避免阶段 1 过早锁死 segment 数量，也能为后续 Single-take Audit 提供因果依据。

阶段 1 的 `timeline_blueprint` 应由 `narrative_events` 推导，而不是直接凭空生成。`timeline_blueprint` 可以是候选时间线，但如果阶段 2B 或 Single-take Audit 发现某个事件内状态变化过大，允许按事件边界或事件内部关键状态重新拆分。

### 5.5 动态状态注册表：Anchor State Timeline

Consistency Manifest 解决“什么不能变化”，但剧情还需要记录“什么可以合理变化，以及如何变化”。例如产品从桌面被拿起、打开、涂抹、放回，这不是一致性漂移，而是合法状态演进。
...
- `consistency_manifest` 锁定身份、外观、品牌、空间结构；
- `anchor_state_timeline` 描述合法状态演进和位置变化。

它直接服务于以下问题：

- 产品复制：桌上已有一个，人物手里又凭空多一个；
- 道具突然出现或消失；
- 人物服装、姿势、持物状态在镜头间无理由变化；
- 场景布局被误认为可以随剧情任意变化；
- 视频模型不知道同一个产品应该从 A 点连续移动到 B 点。

如果某个 anchor 在事件中发生状态变化，阶段 2B 的 `start_frame_contract`、`end_frame_contract` 和 `motion_contract` 必须引用对应的 `AnchorStateTimeline`，并说明这个状态变化在一镜到底中是否可见、是否需要拆段。
...
## 8. 阶段 2：Storyboard Artist + Shot Decomposer 改造

阶段 2 的目标不是写最终 prompt，而是生成渲染可执行的结构化镜头合同。这里不应再让一个模型在同一轮里既制定整体分镜，又展开所有镜头细节，否则容易出现 brief 和 render description 不一致、前几段很详细后几段变简略、机位图在写完镜头后才被补出来等问题。

因此阶段 2 应拆成两个明确子任务：

```text
阶段 2A：Storyboard Artist
  只负责整体分镜：每个 segment 看什么、叙事功能是什么、使用哪个 camera_id、段间如何衔接。

阶段 2B：Shot Decomposer
  只负责逐镜头工程拆解：首帧合同、尾帧合同、运动合同、可见锚点、动态状态、motion checkpoints。
```

这仍然属于“三阶段架构”里的第二阶段，但在工程实现中应作为两个 LLM 子调用或两个严格分隔的 prompt block 执行。2A 的输出是 2B 的输入，2B 不允许擅自重写剧情事件，只能在发现一镜到底不可执行时提出 `timeline_change_request` 或进入 Split Repair。
...
借鉴 ViMax 的 `storyboard.json`，阶段 2A 先输出简洁分镜草案：
...
- 大概声音意图是什么？
- 依赖哪些一致性锚点？
- 它来自哪些 Narrative Event？
- 是否因为事件边界、地点边界、状态边界或机位边界而必须独立成段？

### 8.3 Segment Render Description

借鉴 ViMax 的 `shot_description.json`，阶段 2B 对每个 segment 必须输出：
...
阶段 2B 还必须核对 `AnchorStateTimeline`：如果某个产品、道具、角色姿态或空间状态在本段发生变化，`motion_contract` 必须写清楚它如何从 start state 到 end state；如果无法连续完成，应返回 `requires_cut=true` 或推荐拆分。
...
不要继续使用 `is_parent_fully_covers_child` 这类字段。真实摄影语义里，广角父镜头不一定“完全覆盖”俯拍、特写或反打镜头；它可能只是共享空间、共享主体，或者沿同一轴线重新构图。因此 Camera Graph 应记录关系类型，而不是简单的父子覆盖布尔值。
    ...
    "nodes": [
      {
        ...
        "movement_style": "",
        "spatial_layout_lock": "",
        "relation_to_parent": "same_axis | derived_reframe | same_spatial_context | alternate_view | new_camera_setup",
        "missing_info": [],
...
后续生成 keyframe 和视频 prompt 时，应注入 camera graph 的继承规则。不同关系的用途不同：

- `same_camera_setup`：可以强继承构图、轴线、空间布局和光线；
- `same_axis`：可以继承轴线和空间方向，但允许景别变化；
- `derived_reframe`：可以继承主体关系和空间布局，但必须重写构图边界；
- `same_spatial_context`：只继承地点结构、光线和固定物体；
- `same_subject_group`：只继承主体组合，不继承机位；
- `alternate_view`：需要注意 180 度轴线和左右关系；
- `new_camera_setup`：应要求新的机位参考或 transition reference。

这样 Camera Graph 才能真正参与 180 度轴线控制、构图继承、参考图选择、是否需要新机位参考、是否允许同镜头内连续运镜等判断。
...
但 `transition_reference_plan` 只解决“新机位关键帧生成时如何保持空间一致”，不解决“最终成片里 Segment A 和 Segment B 如何自然衔接”。因此还需要独立的 `final_transition_plan`：
...
- `transition_reference_plan` 是生成前/生成中的参考素材计划，不一定进入最终视频；
- `final_transition_plan` 是最终合成计划，控制段间视觉转场、音频重叠、是否需要桥接片段；
- 如果 `visualMode=generated_bridge`，必须生成额外桥接素材，并经过用户审核；
- 如果 `match_cut` 使用某个产品、人物动作或画面元素做匹配，必须写入 `matchAnchorId`；
- HappyHorse 单个 clip 内仍然禁止 dissolve/hard_cut/match_cut 这类转场词，转场只在最终合成阶段使用。
...
### 10.1.1 Reference Selector 确定性算法

`ReferenceImageSelector` 不能只是“再调用一次大模型，让它选择参考图”。它必须有确定性的候选排序、数量上限和冲突判断。

建议选择优先级：

1. 当前帧可见的 hard identity / product anchor；
2. 同地点、同 camera relation 的最近历史关键帧；
3. 对应朝向的人物多视图参考，例如 front、side、back、three_quarter；
4. 空间布局、父机位或 transition reference；
5. 品牌视觉、构图或风格参考。
...
```ts
interface ReferenceCandidateScore {
  referenceId: string;
...
- hard anchor 可见时，对应身份/产品参考图必须入选，除非用户未生成或未锁定；
- `conflictScore` 高的图不能入选，例如人物不一致、产品 logo 错误、文字错误、场景结构冲突；
- 风格图不能替代身份图或产品图；
- 父机位/transition reference 只能继承空间、构图、光线，不能继承错误人物、错误产品、错误文字；
- 如果候选超过 4 张，先按类型配额保留，再按 finalScore 排序；
- 被淘汰的候选必须记录 `rejectionReason`，方便调试。

执行算法建议：

```text
1. 根据目标帧/检查点读取 visible_anchor_ids、camera_id、location_id、camera_relation、frame_contract。
2. 收集候选：
   - 当前可见 hard anchor 的已锁定参考图；
   - 同 location + same_camera_setup/same_axis/derived_reframe 的最近历史关键帧；
   - 人物多视图中朝向最接近当前构图的一张；
   - 父机位关键帧或 transition_reference；
   - 用户上传风格图和品牌视觉图。
3. 给每个候选打分：
   - relevanceScore：候选用途是否匹配当前 visible anchor / scene layout / brand visual；
   - conflictScore：候选是否包含错误人物、错误产品、错误 logo、错误文字、冲突场景结构；
   - recencyScore：候选距离当前 segment 越近分越高，但不能高过 hard anchor；
   - viewMatchScore：人物朝向、产品角度、机位关系是否匹配当前 frame contract。
4. 先锁定必选图：
   - hard person anchor 最多 1 张；
   - hard product anchor 最多 1 张；
   - 如果两者都可见，二者都优先于风格图。
5. 再按类型配额补足：
   - scene layout 最多 1 张；
   - composition/style/brand visual 最多 1 张；
   - 总数最多 4 张。
6. 如果某个候选 conflictScore 超阈值，即使 finalScore 高也淘汰，并记录 rejectionReason。
7. 如果 hard anchor 缺少已锁定参考图，不能悄悄用风格图替代，应记录 `missingRequiredReference`，必要时阻止生成或回到一致性资产生成。
8. 输出 selected_references、reference_usage_notes、rejection_reason_for_others 和最终 text_prompt。
```

该算法可以先由后端程序完成候选过滤和打分，大模型只负责补充“用途说明”和“忽略哪些参考图内容”。这样选择结果会比纯 LLM 决策稳定。

`reference_selection_outputs` 应扩展为：
...
```json
{
  ...
  "selection_policy_version": "ref-selector-v1",
  "candidate_references": [
    {
      "reference_id": "",
      "source": "anchor_image | user_reference | parent_camera | previous_keyframe | transition_reference | style_reference",
      "url": "",
      "intended_usage": "identity | product | scene_layout | style | composition | brand_visual",
      "relevanceScore": 0,
      "conflictScore": 0,
...
只把结构存进 `planJson` 还不够。要支持真正可靠的局部重试，需要给每个中间产物、图片、视频、prompt 编译结果记录依赖、版本和 dirty 状态。
...
interface ArtifactMetadata {
  ...
  inputHash: string;
  dependsOn: string[];
  producedByStage: "stage_1" | "stage_2a" | "stage_2b" | "single_take_audit" | "stage_3" | "compiler" | "generation" | "quality_eval" | "user_edit";
  status: "draft" | "dirty" | "approved" | "generating" | "ready" | "failed";
  dirtyReason?: string;
  invalidatedByArtifactIds?: string[];
  retryFromStage?: "stage_1" | "stage_2a" | "stage_2b" | "stage_3" | "generation" | "quality_eval";
}
```

使用原则：

- 用户修改 anchor，依赖该 anchor 的 keyframe、motion checkpoint、segment prompt 都应标记 dirty；
- 用户修改某个 `NarrativeEvent`，依赖它的 storyboard brief、segment render description、transition plan 都应标记 dirty；
- 用户修改某个 keyframe prompt，只应影响该关键帧图片和依赖该图的后续视频片段；
- prompt 版本、模型版本、输入哈希变化时，旧产物不能被误认为仍然可复用；
- 每次局部重试必须写明从哪个 artifact revision 派生。
- `retryFromStage` 不应由前端猜测，而应由依赖图推导：如果只是图片失败，从 `generation` 重试；如果 reference selector 错，从 `stage_3/compiler` 重试；如果 segment 合同错误，从 `stage_2b` 重试；如果事件边界错误，回到 `stage_1`。
- 一个 artifact 被用户批准后，后续自动重试不得覆盖它，只能创建新 revision 并等待用户确认。
...
- 每个关键帧、子分镜参考图和视频片段生成后，都应生成 `GenerationQualityReport`。
- 如果生成多个候选结果，应先评分再选择最佳版本，不应默认使用第一个完成的结果。
- 质量报告至少包含身份一致性、空间布局、prompt 对齐、连续性、一镜到底风险、明显伪影等维度。
- 未通过质量检查的资产不能直接进入下一阶段，除非用户明确手动接受。
- 重试时必须把失败原因转成 `retryInstruction`，反馈给下一次图片/视频 prompt 编译。
...
interface GenerationQualityReport {
  assetId: string;
  assetType: "anchor_image" | "keyframe" | "motion_checkpoint_image" | "video_segment" | "final_video";
  candidateId?: string;
...
- 关键帧和子分镜参考图至少做一次视觉理解检查，确认人物/产品/场景/品牌文字是否符合 `referenceSelectionOutputs` 和 frame contract。
- 视频片段至少检查 `singleTakeScore`、首帧一致性、主体连续性、产品/道具实例数、字幕是否误入画面。
- 如果同一资产生成多个候选，先生成多个 `GenerationQualityReport`，再选择 `passed=true` 且综合分最高的候选；不能默认使用第一个返回的结果。
- 未通过但可修复的资产，应把 `artifactIssues` 编译成 `retryInstruction`，传给下一次图片/视频 prompt 编译。
- 未通过且属于结构问题的资产，例如“该 segment 必须切镜才能完成”，不能继续重试生成，应回退到 `stage_2b` 或 Single-take Split Repair。
- 用户可以手动接受未通过资产，但必须把 `passed=false` 和 `userAccepted=true` 写入 metadata，后续调试能看出来这是人为放行。
...
Create narrative_events before creating candidate_timeline.
Create anchor_state_timeline for every anchor that can legally move, change holder, open/close, appear/disappear, or change state.
Do not freeze segment count until narrative_events and state changes are known.
...
Stage 2A must not write full render prompts.
Stage 2B must not redesign the story unless it returns a timeline_change_request.
Every segment must reference source_event_ids and relevant anchor_state_timeline entries.
...
Use CameraRelation values instead of parent fully covers child.
Create final_transition_plan separately from transition_reference_plan.
```
...
Use the deterministic Reference Selector policy:
- max 4 references,
- max 1 person identity reference,
- max 1 product reference,
- max 1 scene layout reference,
- max 1 composition/style/brand visual reference.

Do not blindly inherit every detail from reference images.
Explain what should be inherited and what should be ignored.
Write prompt compilation and reference selection debug artifacts for every generated image/video asset.
```
...
1. 文档和类型层增加 `narrativeEvents`、`anchorStateTimeline`、`audioBible`、`cameraGraph`、`finalTransitionPlan`、`transitionReferencePlan`、`segmentRenderDescriptions`、`motionCheckpoints`。
2. 阶段 1 prompt 增加 `narrative_events`、`anchor_state_timeline`、`audio_bible`、更细的 anchor 类型专属锁定字段、是否需要角色三视图的判断。
3. 阶段 1 的 `candidate_timeline` 必须从 `narrative_events` 推导，不能直接凭空锁死 segment。
4. 阶段 2 拆成 2A/2B 两个子任务：2A 输出 `storyboard_brief` 和机位/转场初稿，2B 输出每段渲染合同。
5. 阶段 2 prompt 增加 `start_frame_contract`、`end_frame_contract`、`motion_contract`、`variation_type`、`visible_anchor_ids`、`camera_id`、`camera_graph`。
6. 后端归一化保留这些新字段到 `planJson`，先不急着新增表。
7. 先实现硬一致性 anchor 图片优先生成和锁定；`person` anchor 可先支持一张主参考图，再扩展三视图。
8. 增加确定性 `Reference Selector`：候选收集、四类配额、四个分数、冲突淘汰、缺失 hard reference 阻断。
9. 增加 `referenceSelectionOutputs` 调试结构，记录关键帧/子分镜图生成时的候选参考图、选中参考图、用途说明和最终 text prompt。
10. 改造图片 Prompt Compiler：最终图片 prompt = 参考图用途说明 + 静态帧描述 + text prompt + negative prompt，而不是直接使用 keyframe image prompt。
11. 普通关键帧生成前，强制等待 hard anchor 参考图生成并锁定；生成时只传当前帧实际可见锚点相关参考图。
12. Prompt compiler 注入 `cameraGraph`、`visible_anchor_ids`、`singleTakeContract`、`audioBible` 和 `referenceSelectionOutputs`。
13. 将 `microShots` UI 文案逐步迁移为“子分镜 / 运动检查点”，内部结构优先使用 `motionCheckpoints`。
14. 增加 Single-take Audit，失败时 Split Repair，达到最大轮次仍失败则阻止生成。
15. 增加父机位继承策略：短期用父关键帧作为候选参考图；中期实现 `transitionReferencePlan`，用 transition video 抽帧作为新机位参考图。
16. 增加 `finalTransitionPlan` 合成策略：视觉转场、音频 J-cut/L-cut/crossfade、是否生成 bridge 素材。
17. 增加视频 Prompt Compiler 规则：视频 prompt 主要使用 `motion_contract + single_take_contract + motion_checkpoints`，不要重复堆完整 visual desc。
18. 增加音频后期策略：剥离片段原音轨，统一加 BGM/TTS/SFX。
19. 增加 `artifactMetadata` 依赖图：revision、inputHash、dependsOn、dirtyReason、retryFromStage。
20. 增加 `GenerationQualityReport`：关键帧、子分镜图、视频片段生成后都要评估，失败原因转为 retryInstruction。
21. 增加调试 UI 或日志入口：展示某张图的 `referenceSelectionOutputs`、最终 prompt、选中参考图、被忽略参考图原因和质量报告。
22. 后续再考虑新增数据库表，把 anchor、camera、checkpoint、reference selection、prompt compilation、audio asset、quality report 从 `planJson` 中拆出来。
...
```text
1. 阶段 1 输出 narrativeEvents 和 anchorStateTimeline。
2. 阶段 2B 输出 visible_anchor_ids / start_frame_contract / end_frame_contract / motion_contract。
3. 后端保存 narrativeEvents、anchorStateTimeline、segmentRenderDescriptions。
4. 图片生成前根据 visible_anchor_ids 和 deterministic Reference Selector 选择 anchor 参考图。
5. 记录 referenceSelectionOutputs。
6. 视频 prompt 改为优先使用 motion_contract。
```

这一批不需要马上做 transition video、三视图、TTS、质量评估或新增数据库表，但已经能明显提升人物/产品一致性、状态连续性和一镜到底可控性。

### 15.2 第二批落地顺序

```text
1. 阶段 2 拆成 Storyboard Artist / Shot Decomposer 两个子调用。
2. person anchor 支持 front / side / back / three_quarter 多视图。
3. cameraGraph 参与关键帧参考图选择。
4. 父机位关键帧作为新机位候选参考图。
5. promptDebugArtifacts 展示到前端调试窗口。
6. Single-take Audit 和 Split Repair 阻断高风险 segment。
7. artifactMetadata 记录 revision、dependsOn、dirtyReason、retryFromStage。
```
...
3. finalTransitionPlan 驱动最终合成里的视觉转场和音频 J-cut/L-cut/crossfade。
4. 建立统一音频后期链路：BGM / TTS / SFX / loudnorm / ducking。
5. 引入 GenerationQualityReport，对关键帧、子分镜图和视频片段做质量评估与候选选择。
6. 将高频调试产物拆出独立数据表，支持局部重跑和版本比较。
```
