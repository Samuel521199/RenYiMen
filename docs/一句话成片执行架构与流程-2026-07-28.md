...
> 文档版本：2026-07-29（第三次复核，按当前工作区代码与本地运行配置更新）
>
> 适用范围：`RenYiMen` 一句话成片（One Prompt Video）工作流
>
> 本文只描述当前实际执行路径，不保留历史架构说明。凡“当前本地配置”均指本次复核时的 `.env.local` 非敏感开关快照；生产默认和本地快速预览会明确分开说明。

## 1. 一句话总览

当前“一句话成片”是一套以结构化创作合同为核心、以审核门为标准生产边界、以目标级持久化作业和独立 Worker 为执行底座、以 Provider 能力声明适配不同生成模型的视频生产系统。开发环境还提供受生产保护的快速预览模式，用于自动跨过审核门验证端到端链路。
...
flowchart TD
    ...
    C --> C1["参考图客观事实提取<br/>保存结构化事实与指纹"]
    C1 --> C2["构造结构化模型输入<br/>直接传入用户原文、参考事实与合同"]
    ...
    subgraph STORY_GATES["阶段 4｜故事与资产门禁"]
        direction TB
        SG1["① Story Contract 检查<br/>事件绑定、节拍顺序、因果引用、证据引用、模板必需职责"]
        SG2{"② 合同是否有效？"}
        SG3["③ SCRIPT_QA 开启时<br/>只修复 storyBeats 与引用关系"]
        SG4["④ SCRIPT_QA 开启时执行语义质量评审<br/>Hook、冲突、因果、转折、兑现、CTA 与参考图转化"]
        SG5{"⑤ 是否存在高置信度语义问题？"}
        ...
        SG1 --> SG2
        SG2 -- "否且 SCRIPT_QA 开启" --> SG3 --> SG1
        SG2 -- "是且 SCRIPT_QA 开启" --> SG4 --> SG5
        SG2 -- "SCRIPT_QA 关闭，仅记录报告" --> SG7
        SG5 -- "是，可修复" --> SG6 --> SG4
...
flowchart TB
    ...
    C["参考图事实提取<br/>构造结构化模型输入"]
    D["阶段 1<br/>Route → Planning Architect"]
    E["按 anchor 并发生成视觉规格"]
...
这部分已经不是待实施方案，而是当前正式路径：

- 当前固定在 `percent_100`，`Route Contract` 是唯一分类权威；
- 路由模型为 `qwen3.7-plus`，温度 `0.1`，关闭 thinking，`max_tokens=450`；
- 单次正常调用硬超时 20 秒，UTF-8 返回硬上限 2KB，最多允许一次受限模型修复；
- Route Gate 先做 JSON、版本、字段、枚举、category/template、chronology/Hook、置信度、越权字段和事件 ID 检查，再决定允许、程序修复、模型修复或安全回退；
- `route_classification` checkpoint 保存输入/参考事实指纹、模型耗时、token、Gate、修复和 fallback；输入未变化时模型调用数为 0；
- 用户可在 PLAN_REVIEW 修改路线并锁定为 `authority=user`，之后模型不得覆盖；
- 最终计划中的分类字段只镜像 `approvedRouteContract`，Planning Architect 不再拥有旧分类职责。

Tongits King 的最终 20 次真实复验结果为 category/template/chronology 一致率均 100%，P50 6.177 秒、P95 8.001 秒、最大返回 1,177 bytes、正常修复调用 0。该数字是固定样本专项复验基线，不等同于整个 Planning Architect 的总耗时。
...
%%{init: {"themeVariables": {"fontSize": "18px"}}}%%
flowchart TB
    Q{"ONE_PROMPT_VIDEO_SCRIPT_QA<br/>是否开启？"}
    G1["① Story Contract 检查<br/>事件绑定 · Beat 顺序 · 因果/证据引用<br/>模板必需叙事职责"]
    G2{"② 合同有效？"}
    ...
    ADVISORY["只执行检查并记录 advisory 报告<br/>不调用剧情 Critic，不执行模型返修"]
    ...
    class Q,G1,G2,G4,G5,G7,G8,NEXT,ADVISORY gate;
    class G3,G6 repair;
```
...
自 2026-07-29 起，规划链路不再使用 Qwen-MT 或 LibreTranslate 翻译边界。模型直接接收系统 Prompt、用户原文和结构化合同，返回结果也直接进入合同校验与后续流水线。中英文字段由规划 Prompt 和结构化 Schema 约束，不再通过额外翻译模型补写。

对应的本地翻译实现、启动脚本、环境变量和 Docker 服务已经移除，并有 `translation-removal.test.ts` 防止旧翻译边界被重新引入。这里的“英文执行 Prompt”是从结构化合同确定性编译出来的生产文本，不代表系统仍在运行一个独立翻译服务。
...
标准生产模式下，PLAN_REVIEW、资产审核、边界关键帧审核、MICRO_SHOT_REVIEW、CLIP_REVIEW 和 FINAL_REVIEW 都是真实人工门。
...
- 在依赖产物已经齐备时自动批准计划、资产库、全部边界关键帧、子分镜、视频片段和最终成片；
- 对已经返回且可用的媒体候选自动选择，不等待完整模型视觉评分；
- 转场参考使用 `short` 模式，最终合成跳过生成式桥接视频；
- Script QA 被强制关闭；Planning/Storyboard 的剧情合同检查只保存 advisory 报告，不调用剧情 Critic 或模型返修；
- JSON 可解析性、结构化 Schema、必填字段、Route Gate、Provider 输入能力、Prompt 硬预算、目标修订和 lease 所有权等工程门禁仍然执行。

快速预览不是生产默认：`fast-preview-config.ts` 在 `NODE_ENV=production` 时即使误配为 `true` 也会返回 `production_blocked`。因此本文后续描述审核门时默认指标准生产流程；当前本地端到端试跑则按本节的自动推进覆盖规则执行。
...
`creativeStrategy` 在同一条事件链上确定受众与核心承诺，并把 Hook、冲突、转折、兑现和 CTA 绑定到具体事件。叙事模板与时间顺序不再由 Planning Architect 决定，而是只读镜像 `approvedRouteContract`；它既不能改分类，也不能另外创造一套与 `narrativeEvents` 不一致的故事。这样可以避免“结果突然出现”“人物无动机行动”“产品能力没有被可见动作证明”或“CTA 早于剧情兑现”等问题。
...
- Shot Decomposer 等通用结构化阶段经过泛型 `executeStructuredStage<T>()`；独立 Route 使用自己的 20 秒/2KB 调用器和 Route Gate；
...
- 输入快照与参考图指纹
- `routeClassification`、只读 `approvedRouteContract`、Route Gate/修复/fallback/人工锁定信息
- 结构化失败指纹
- migration audit
...
“驱动最终生成”在这里包含两层含义：它们决定动作检查点、图片生产和 `videoPromptContract` 的内容；是否把对应图片真的传给视频模型，则由 Provider 能力决定。当前 Wan 2.7 原生首尾帧模式只传首帧和尾帧。
...
### 7.3 当前默认：Wan 2.7 原生首尾帧

当前默认视频模型已经切换为 `wan2.7-i2v-2026-04-25`，并且在未显式允许自定义模型时由代码强制使用该型号。默认 Provider 合同为：

| 项目 | 当前值 |
|---|---|
| Provider | `ALIYUN_BAILIAN` / DashScope |
| 输入模式 | `native_first_last` |
| 原生图片输入 | 已批准首帧 + 已批准尾帧，共 2 张 |
| Prompt 引用模式 | `plain_action`，不使用 `[Image N]` 多图编号教学 |
| 分辨率 | `720P` |
| 时长范围 | 2–15 秒，按 Segment 合同裁定 |
| `prompt_extend` | Wan 2.7 固定关闭 |
| Prompt 硬上限 | 5,000 字符 |
| negative prompt | 最多 500 字符 |

Wan 编译器把 `videoPromptContract` 编译为“两个固定视觉边界之间的连续运动”：

```text
FIRST_FRAME 是精确开场画面
→ 按 motionSteps 完成连续可见动作
→ 满足 terminalRequirements
→ 全程遵守 preserveRequirements / forbiddenOutcomes
→ 减速并精确落到 LAST_FRAME
```

`resolvedMicroShots` 仍是权威动作检查点和生产/审核记录，也会参与动作合同与 Prompt 编译；但在当前 `native_first_last` 模式下，它们不会作为第 3 张及之后的图片传给 Wan 2.7。首尾帧承载身份与场景连续性，中间动作语义由 `videoPromptContract` 表达。

### 7.4 可选模型兼容

HappyHorse 等多参考图模型仍保留在能力层和编译器中，但只有显式开启 `ALIYUN_I2V_ALLOW_CUSTOM_MODEL=true` 并选择对应模型/输入模式后才会生效。支持有序多图时可使用自然语言 `[Image N]` 关系；不支持编号引用时退化为普通动作 Prompt；无法满足硬首尾帧或必需锚点能力时阻止提交并返回合同修复。
...
- 路由分类读取/人工修改：`/video-projects/[projectId]/route-classification`
...
- 独立 `planning-route-v1`、Route Gate、受限修复、安全回退、指纹复用、人工锁定和 `percent_100` 唯一分类权威；
- 规划模型直接接收用户原文和结构化合同，本地翻译服务及其运行配置已彻底移除；
- 资产中文展示副本与英文执行合同分离；
...
- Wan 2.7 原生首尾帧作为默认视频生成合同，HappyHorse 等作为显式自定义兼容路径；
- 受生产环境保护的 Fast Preview，以及独立的 Script QA 开关；
...
- 在标准生产模式中，自动视频审查主要覆盖技术质量和末帧连续性，完整语义审计仍是按需建议性报告；
- 标准生产模式仍保留计划、资产、关键帧、子分镜、视频片段和最终成片等人工审核门；当前本地 Fast Preview 会在产物齐备后自动通过这些门；
- Script QA 当前默认关闭，本地配置也为关闭；剧情合同与语义问题会记录为 advisory，不触发额外 Critic/返修调用；
...
以上是标准生产用户路径。当前本地 Fast Preview 会在每一级依赖产物齐备后自动执行“审核/选择/继续”，所以本地试跑的可见行为更接近“输入后由 Worker 自动推进到 DONE”；自动推进不会改变底层业务状态机，日志中会记录每次 `fast_preview.review_auto_approved` 和候选自动选择。
...
| Route 输入、模型调用、Gate、修复、checkpoint、遥测、人工审核与灰度 | `src/services/video-orchestrator/planning-route-*.ts` |
| Fast Preview 的生产保护与读取 | `src/services/video-orchestrator/fast-preview-config.ts` |
| Script QA 与故事门禁开关 | `src/services/video-orchestrator/script-qa-config.ts`、`story-rollout-config.ts` |
...
| Wan 2.7 / HappyHorse 视频 Prompt 编译 | `src/services/video-orchestrator/video-terminal-contract.ts` |
| 阿里云图片/视频模型选择、能力声明与提交装配 | `src/services/video-orchestrator/aliyun-workflow.ts` |
...
5. 默认用 Wan 2.7 原生首尾帧执行，并通过 Provider 能力层保留其他模型适配；
