...
@/Volumes/AIWork/projects/ai-image-workbench/get-shit-done/references/agent-contracts.md
@/Volumes/AIWork/projects/ai-image-workbench/get-shit-done/references/context-budget.md
@/Volumes/AIWork/projects/ai-image-workbench/get-shit-done/references/gates.md
</required_reading>
...
**Model resolution:** If `executor_model` is `"inherit"`, omit the `model=` parameter from all `Task()` calls — do NOT pass `model="inherit"` to Task. Omitting the `model=` parameter causes Cline to inherit the current orchestrator model automatically. Only set `model=` when `executor_model` is an explicit model name (e.g., `"claude-sonnet-4-6"`, `"claude-opus-4-7"`).
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @/Volumes/AIWork/projects/ai-image-workbench/get-shit-done/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @/Volumes/AIWork/projects/ai-image-workbench/get-shit-done/references/planner-antipatterns.md
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `/Volumes/AIWork/projects/ai-image-workbench/get-shit-done/workflows/execute-plan.md` **inline**
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Task(
     ...
     prompt="
       ...
       @/Volumes/AIWork/projects/ai-image-workbench/get-shit-done/references/checkpoints.md
       @/Volumes/AIWork/projects/ai-image-workbench/get-shit-done/references/tdd.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@/Volumes/AIWork/projects/ai-image-workbench/get-shit-done/references/executor-examples.md'}
       </execution_context>
...
Read and follow `/Volumes/AIWork/projects/ai-image-workbench/get-shit-done/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
