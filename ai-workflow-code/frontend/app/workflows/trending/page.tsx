                ...
                <div className="grid max-h-64 grid-cols-4 gap-2 overflow-y-auto pr-1">
                  {refAssets.map((asset) => {
                    ...
                    return (
                      ...
                      >
                        <img
                          src={`${process.env.NEXT_PUBLIC_API_BASE || "http://localhost:8000"}${asset.url}`}
              ...
              {ws.finalImages
                ...
                .map((img) => {
                  const fullUrl = img.url.startsWith("http")
                    ? img.url
                    : `${process.env.NEXT_PUBLIC_API_BASE || "http://localhost:8000"}${img.url}`;
                  return (
                    <div
                      key={img.id}
          ...
          <img
            src={
              previewUrl && previewUrl.startsWith("http")
                ? previewUrl
                : `${process.env.NEXT_PUBLIC_API_BASE || "http://localhost:8000"}${previewUrl}`
            }
            alt={t("预览")}
