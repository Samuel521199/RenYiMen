...
test("project integration preserves front-first derivation and approved media", () => {
  ...
  assert.match(source, /const dependencyReadyAssetKeyframes = nextKeyframes\.filter[\s\S]*?isAssetViewGenerationReady/);
  assert.match(source, /waitingForConsistencyReferences && !running\.length[\s\S]*?status: VideoProjectStatus\.IMAGE_REVIEW/);
