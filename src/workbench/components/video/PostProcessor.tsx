...
import { buildSubtitleSegments, splitSubtitleLines } from "@workbench/lib/video-automation";

import { useCallback, useEffect, useRef, useState } from "react";

import { useLanguage } from "@workbench/lib/LanguageContext";
import type { PostConfig } from "@workbench/lib/video-workflow";
...
function LogoPickerModal({
  ...
  useEffect(() => {
    ...
    workbenchFetch("/api/assets?category=logo&page=1&page_size=20", {
      ...
      .then((res) => {
        if (res?.code === 0) {
          setItems(
            parseAssetListResponse(res.data).map((asset) => ({
              id: asset.id,
              url: asset.url?.startsWith("http") ? asset.url : `/api/workbench${asset.url}`,
            })),
          );
        }
      })
...
export default function PostProcessor({
  ...
  const update = useCallback(
    <K extends keyof PostConfig>(layer: K, value: Partial<PostConfig[K]>) => {
      const next = {
        ...configRef.current,
        [layer]: {
  ...
  const subtitleTemplateStyle = (() => {
    const template = config.subtitle.styleTemplate ?? "social_pop";
        ...
        >
          <span
            style={{
              ...
              borderRadius: subtitleTemplateStyle.borderRadius,
              background: subtitleTemplateStyle.background,
              color: subtitleTemplateStyle.color,
              padding: "4px 12px",
              fontSize: `${config.subtitle.fontSize}px`,
              whiteSpace: "pre-line",
            }}
          >
            {(config.subtitle.lines?.length ? config.subtitle.lines.join("\n") : config.subtitle.text).trim()}
          </span>
        </div>
      )}
            ...
            {config.subtitle.enabled && (
              <div className="space-y-2 border-t border-gray-100 pt-1">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <select
                    value={config.subtitle.styleTemplate ?? "social_pop"}
                    onChange={(event) =>
                      update("subtitle", {
                        styleTemplate: event.target.value as "social_pop" | "minimal_clean" | "news_banner",
                      })
                    }
                    className="rounded-lg border border-gray-200 bg-white px-2 py-1.5 text-xs"
                  >
                    <option value="social_pop">社媒高能（默认）</option>
                    <option value="minimal_clean">极简清爽</option>
                    <option value="news_banner">资讯横幅</option>
                  </select>
                  <select
                    value={config.subtitle.maxCharsPerLine ?? 14}
                    onChange={(event) => update("subtitle", { maxCharsPerLine: Number(event.target.value) })}
                    className="rounded-lg border border-gray-200 bg-white px-2 py-1.5 text-xs"
                  >
                ...
                <button
                  onClick={() => {
                    const maxChars = config.subtitle.maxCharsPerLine ?? 14;
                    const lines = splitSubtitleLines(config.subtitle.text, maxChars);
                    const segments = buildSubtitleSegments(config.subtitle.text, duration || 5, maxChars);
                    update("subtitle", { lines, segments });
                  }}
