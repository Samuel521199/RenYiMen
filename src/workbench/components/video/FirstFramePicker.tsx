...
function resolveAssetUrl(rawUrl: unknown): string {
  ...
  return workbenchStaticUrl(value);
}
...
export default function FirstFramePicker({ token, onSelect, onClose }: Props) {
  ...
  const confirmSelection = (item: AssetItem) => {
    onSelect({
      asset_id: item.id,
      url: item.url,
  ...
  return createPortal(
    ...
    >
      <div
        className="relative flex w-[860px] max-w-full flex-col overflow-hidden rounded-2xl bg-white shadow-2xl"
        style={{ height: "min(80vh, calc(100vh - 2rem))" }}
      ...
      >
        ...
        <div className="flex shrink-0 items-center gap-4 border-b border-gray-100 px-6 py-3">
        ...
        <div className="flex shrink-0 items-center justify-between border-t border-gray-100 bg-white px-6 py-4">
          <div className="min-w-0 pr-4 text-sm text-gray-500">
            {selected
              ? `${t("已选择")} · ${selected.source_type} #${selected.id} · ${t("再次点击或点确认")}`
              : t("点击图片选择首帧")}
          </div>
