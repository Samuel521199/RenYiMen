...
import {
  ...
  type AssetGridSize,
} from "@workbench/lib/asset-grid";
import { ASSET_CATEGORIES } from "@workbench/lib/constants";
import { parseAssetListPageResponse } from "@workbench/lib/asset-list";
import { apiDelete, apiGet, apiPatch, apiUpload, workbenchStaticUrl } from "@workbench/lib/api";
...
import type { Asset, AssetCategory, AssetStats } from "@workbench/lib/types";
...
interface AssetPaginationBarProps {
  t: (value: string) => string;
  assetPage: number;
...
function AssetPaginationBar({
  ...
  onPrev,
  onNext,
}: AssetPaginationBarProps) {
  if (assetTotal <= 0) return null;

  return (
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-gray-200 bg-white px-4 py-3 text-sm shadow-sm">
      ...
      {totalAssetPages > 1 && (
        <div className="flex items-center gap-2">
          <button
            ...
            onClick={onPrev}
            className="rounded-md border border-gray-300 bg-white px-4 py-2 text-sm text-gray-700 transition hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {t("上一页")}
          </button>
          <button
            type="button"
            disabled={assetPage >= totalAssetPages || loading}
            onClick={onNext}
            className="rounded-md border border-gray-300 bg-white px-4 py-2 text-sm text-gray-700 transition hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {t("下一页")}
...
export default function AssetsPage() {
  ...
  const totalAssetPages = Math.max(Math.ceil(assetTotal / ASSETS_PAGE_SIZE), 1);
  ...
  async function loadAssets(
    ...
    try {
      ...
      params.set("page_size", String(ASSETS_PAGE_SIZE));
      if (nextCategory !== "all") params.set("category", nextCategory);
      if (nextTags.length > 0) params.set("tags", nextTags.join(","));
      const res = await apiGet<unknown>(`/api/assets?${params.toString()}`);
      if (res.code !== 0) {
        setError(res.msg || t("素材列表加载失败"));
        return;
      }
      const pageData = parseAssetListPageResponse(res.data, ASSETS_PAGE_SIZE, nextPage);
      setAssets(pageData.items);
      setAssetTotal(pageData.total);
      setAssetPage(pageData.page);
    } catch {
      setError(t("无法连接后端服务"));
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    loadAssets(category, selectedTags, assetPage);
    // eslint-disable-next-line react-hooks/exhaustive-deps -- load when filters or page changes
  }, [category, selectedTags, assetPage]);
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      const nextTotal = Math.max(assetTotal - 1, 0);
      const nextMaxPage = Math.max(Math.ceil(nextTotal / ASSETS_PAGE_SIZE), 1);
      if (assetPage > nextMaxPage) {
        setAssetPage(nextMaxPage);
      } else {
        await loadAssets(category, selectedTags, assetPage);
      }
      await loadAssetStats();
      await loadFilterTags(category);
      await loadUploadTagOptions(uploadCategory);
  ...
  return (
    <div>
      ...
      {loading && safeAssets.length === 0 ? (
        ...
        <>
          {loading && (
            <div className="mb-3 rounded-md border border-blue-200 bg-blue-50 px-4 py-2 text-sm text-blue-700">
              {t("正在加载素材...")}
            </div>
          )}
          <div className="mb-4">
