...
import TagCombobox from "@workbench/components/common/TagCombobox";
...
import ImageReviewCard from "@workbench/components/workflow/ImageReviewCard";
...
import WhitespacePositionPicker from "@workbench/components/workflow/WhitespacePositionPicker";
...
import { apiGet, apiPatch, apiPost, apiUpload } from "@workbench/lib/api";
...
function BackgroundWorkflowPageContent() {
  ...
  async function loadAvailableModels() {
    ...
    try {
      ...
      const preferredId = modelConfigId ?? batch?.model_config_id ?? null;
      const resolvedId = preferredId
        ? Number(resolveSelectedModelId(allModels as AvailableExpressionModel[], preferredId) || "") || null
        : null;
      const nextSelectedModelId = resolvedId ?? allModels[0]?.id ?? null;

      setModelConfigId(nextSelectedModelId);
      if (preferredId != null && nextSelectedModelId != null && preferredId !== nextSelectedModelId) {
        setModelSelectionWarning(t("已选模型不可用，已自动切换"));
      } else {
        setModelSelectionWarning("");
      }
      if (allModels.length === 0) {
        setError(t("暂无可用图片模型，请联系管理员分配模型权限"));
      }
    } catch (err) {
      setAvailableModels([]);
  ...
  async function handleCreateBatch() {
    ...
    try {
      ...
      if (createdBatch.session_id) {
        router.replace(`/workbench/workflows/background?session_id=${createdBatch.session_id}`);
      }
      setStep(2);
                        ...
                        {pendingImages.map((image) => {
                          ...
                          return (
                            <ImageReviewCard
                              ...
                              disabled={workingImageId === imageId || isArchivedBatch || generating}
              ...
              <div className="space-y-4">
                ...
                {pendingArchiveImages.length === 0 && (
                  <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-5">
                    ...
                    <button
                      type="button"
                      onClick={() => router.push("/workbench/workflows")}
