...
import { apiDelete, apiGet, apiPost } from "@workbench/lib/api";
...
function sessionTitle(session: WorkflowSession, t: (value: string) => string) {
  ...
  if (session.workflow_type === "multi_fusion") {
    return state.taskName || `${t("多图融合")} #${session.session_id || session.id}`;
  }
...
function getSessionHref(session: WorkflowSession): string {
  ...
  if (session.workflow_type === "daily_post") {
    return `/workbench/workflows/daily-post?session_id=${sessionId}`;
  }
  if (session.workflow_type === "trending") {
    return `/workbench/workflows/trending?session_id=${sessionId}`;
  }
  if (session.workflow_type === "trending_news") {
    return `/workbench/workflows/trending-news?session_id=${sessionId}`;
  }
  if (session.workflow_type === "activity") {
    return `/workbench/workflows/activity?session_id=${sessionId}`;
  }
  if (session.workflow_type === "background") {
    return `/workbench/workflows/background?session_id=${sessionId}`;
  }
  if (session.workflow_type === "multi_fusion") {
    return `/workbench/workflows/multi-fusion?session_id=${sessionId}`;
  }
  return `/workbench/workflows/expression?session_id=${sessionId}`;
}
...
export default function WorkflowsPage() {
  ...
  async function copySession(session: WorkflowSession, targetMode: SessionMode) {
    ...
    try {
      ...
      router.push(`/workbench/workflows/expression?session_id=${newSessionId}`);
    } catch (err) {
