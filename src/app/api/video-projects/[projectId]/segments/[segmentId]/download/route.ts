...
import { getVideoSegmentClipForDownload } from "@/services/video-orchestrator/project-service";
...
export async function GET(_req: Request, ctx: RouteContext) {
  try {
    ...
    const clip = await getVideoSegmentClipForDownload(session.user.id, projectId, segmentId);
    if (isPublicAssetPath(clip.clipUrl)) {
      return downloadPublicAsset(clip.clipUrl, buildClipFilename(clip.title, clip.segmentNo));
    }
    ...
    const filename = buildClipFilename(clip.title, clip.segmentNo);
    ...
    console.error("[video-projects] segment download failed", error);
...
function buildClipFilename(title: string, segmentNo: number): string {
  const safeTitle = title
    .replace(/[\\/:*?"<>|]+/g, "_")
    .replace(/\s+/g, "_")
    .slice(0, 48)
    .replace(/^_+|_+$/g, "");
  return `${safeTitle || "one-prompt-video"}-segment-${String(segmentNo).padStart(2, "0")}.mp4`;
}
